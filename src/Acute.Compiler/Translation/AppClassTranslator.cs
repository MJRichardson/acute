...
namespace Acute.Compiler.Translation
{
    internal class AppClassTranslator : ClassTranslator
    {
        public override void Translate(Blade.Compiler.Models.ClassDeclaration model, TranslationContext context)
        {
            ...
            //write the standard blade transformation of the class
            ...
            context.Write(new module(model.Definition.GetFullName() + "Module", 
                ctorBody, 
                ctor.Parameters.Select(p => p.Definition.Type.GetFullName()).ToArray(), 
                ctor.Parameters.Select(p => p.Name).ToArray()  )
            .TransformText());
